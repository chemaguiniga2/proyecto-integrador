...
async function dcharts() {
    ...
    dchdt.set('clouds', data.clouds)
		cdash1(data.clouds);
		data = JSON.parse(await chartdata("getservices"));
		if (!data.success) {
      dchdt.set('services', null)
			dchartsnds("cdash2");
		} else {
      dchdt.set('services', data.services)
      cdash2(data.services);
      data = JSON.parse(await chartdata("getresources"));
      if (!data.success) {
        dchdt.set('sr', null)
      } else {
        dchdt.set('sr', data.sr)
      }
		
		}
		data = JSON.parse(await chartdata("getresources"));
		if (!data.success) {
      dchdt.set('resources', null)
			dchartsnds("cdash3");
		} else {
      dchdt.set('resources', data.resources)
