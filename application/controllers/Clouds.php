...
class Clouds extends CI_Controller {
  ...
  public function flushregionvm() {
    ...
    $dregion = $this->db->select('id')->from('regions')->where('code', $region)->get()->row();
    $this->db->where('id_user', $iduser)->where('id_region', $dregion->id)->where('id_account', $idaccount)->where('id_type', 1)->delete('resources');
