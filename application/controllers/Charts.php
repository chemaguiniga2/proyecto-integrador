...
class Charts extends CI_Controller {
  ...
  public function getsr()
	{
    $slist = array();
    $services = $this->db->select('*')->from('service_resource')->where('id_user', $this->session->userdata('id'))->get()->result();

    foreach ($services as $service) {
      $this->db->where('id_service', $service->id);
      $num_rows = $this->db->count_all_results('service_resource');
      array_push($slist, array("id_service"=>($service->id_service), "id_resource"=>($service->id_resource)));
    }

    if (count($slist) > 0) {
      echo json_encode(array("success"=>true, "sr"=>$slist));
    } else {
      echo json_encode(array("success"=>false));
    }
    //echo json_encode($response);
