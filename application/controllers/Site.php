...
class Site extends CI_Controller {
  ...
  public function logout() {
    ...
    if (($this->input->post('username') == NULL) || ($this->input->post('pass') == NULL) || ($this->input->post('email')) == NULL) {
      ...
      $user = $this->db->select('*')->from('users')->where('username', $username)->where('email', $email)->get()->row();
      if ($user) {
  ...
  public function passwordrecovery() {
    if ($this->input->post('email') == NULL) { 
      ...
      $email = $this->input->post('email');
      $user = $this->db->select('*')->from('users')->where('email', $email)->get()->row();
      if ($user) {
        $str=rand(); 
        $linkstr = md5($str); 
        $toinsert = array(
  ...
  public function recover() {
    if (($this->input->post('uid') != NULL) && ($this->input->post('pass') != NULL) && ($this->input->post('sresponse') != NULL)) {
      ...
      $response = $this->db->select('*')->from('sresponses')->where('id', $qid)->where('id_user', $uid)->get()->row();
      if ($response) {
      ...
      $user = $this->db->select('*')->from('users')->where('pwrecovery', $key)->get()->row();
      if ($user) {
  ...
  public function mconfirm() {
    $key = $this->input->get('key');
    $user = $this->db->select('*')->from('users')->where('mailconfirm', $key)->get()->row();
    if ($user) {
      $toupdate = array(
        'mailconfirm' => '',
        'securitylevel' => 1
  ...
  public function getmessages() {
    $messages = $this->db->select('*')->from('messages')->where('id_user', $this->session->userdata('id'))->get()->result();
    $notifications = [];
    foreach ($messages as $message) {
      array_push($notifications, array('id'=>$message->id, 'message'=>$message->message));
    }
    echo json_encode(array('success'=>true, 'notifications'=>$notifications));
  }

  public function dispatchmessage() {
    $this->db->delete('messages', array('id' => $this->input->get('id')));
  ...
  public function getfeed() {
    $feed = $this->db->query('select * from `1_user_feed` where id_user=' . $this->session->userdata('id') . ' order by id desc limit 20')->result();
    $pfeed = [];
    foreach ($feed as $row) {
      $ddif = time() - $row->time;
      $sdif = "";
    ...
    echo (json_encode(array('success'=>true, 'feed'=>$pfeed)));
  }
